...
enum MonsterEnum implements Serializable
{
    ...
    BAT (new Monster("Shikaka", "The white flying bat.", GameTextASCII.getBat(), 25, ((int)(Math.random()*4+3)))),  //calls constructor with value 2
    ...
    HAMSTER (new Monster("Hamster", "Don't be fooled by its furry look, its a rapid killer.", GameTextASCII.getHamster(), 150, ((int)(Math.random()*5+5)))),
    ...
    CUPID (new Monster("Cupid", "This little devil is the root of all evil.", GameTextASCII.getCupid(), 75, ((int)(Math.random()*2+1)))),
    SANTA (new Monster("Santa Claus", "The biggest lie you tell your children.", GameTextASCII.getSanta(), 150, ((int)(Math.random()*12+7)))),
    WHALE (new Monster("Moby Dick", "The great white whale from the famous Herman Melville book.", GameTextASCII.getWhale(), 300, ((int)(Math.random()*5+5)))),
    MERMAID2 (new Monster("Mermaid nr. 2", "This even more evil sister of Ariel.", GameTextASCII.getMermaid2(), 150, ((int)(Math.random()*10+5)))),
    RABBIT (new Monster("Killer Rabbit of Caerbannog", "Rabbit of Caerbannog from the AWSOME movie \"Monty Python and the Holy Grail.\"", GameTextASCII.getRabbit(), 300, ((int)(Math.random()*10+5)))),
    ...
    JACKINABOX (new Monster("Jack In A Box", "", GameTextASCII.getJackInABox(), 66, ((int)(Math.random()*3+1)))),
    DUMBO (new Monster("Dumbo", "", GameTextASCII.getDumbo(), 50, ((int)(Math.random()*4+2)))),
    FAIRY (new Monster("Fairy", "", GameTextASCII.getFairy(), 100, ((int)(Math.random()*3+8)))),
    ...
    CENTUAR (new Monster("Posh Centuar", "", GameTextASCII.getCentuar(), 150, ((int)(Math.random()*10+20)))),
    ALIEN (new Monster("Alien", "", GameTextASCII.getAlien(), 78, ((int)(Math.random()*10+5)))),
    GHOST (new Monster("Slimer", "The Jar Jar Binks of the Ghostbusters universe.", GameTextASCII.getGhost(), 100, ((int)(Math.random()*10+3)))),
    ...
    {
        // And here we return a new instance of our merchant from our Enumlist. Where it uses the reference MonsterEnum monsterValues
        return new Monster("Lucifer", "The LORD of the Underworld, The one true evil.", GameTextASCII.getDevil3(), 1000 + GameSettings.getLuciferHealth(), 30 + GameSettings.getLuciferPower());
    }
}
